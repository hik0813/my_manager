...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  return handle(async () => {
    ...
    // 만든 행 이력을 fields에 남겨둔다 — 다음 재분류가 고아 없이 정리할 수 있게
    const cur = await sb.from("captures").select("fields").eq("id", id).single();
    const fields = { ...(((cur.data as { fields?: Record<string, unknown> } | null)?.fields) ?? {}), _created: target.created };

    const res = await sb
      .from("captures")
      .update({ type, target_table: target.table, target_id: target.id, fields, error: null })
      .eq("id", id)
      .select("*")
      .single();
