  ...
  return (
    <>
      <button
        ...
        className="fixed right-4 z-40 tap press h-14 w-14 rounded-full bg-accent text-bg text-2xl font-light shadow-lg flex items-center justify-center"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/60" onClick={() => setOpen(false)}>
          <div
            className="bg-surface border-t border-line rounded-t-2xl px-4 pt-3 pb-4 safe-bottom max-h-[85dvh] flex flex-col"
                  ...
                  {recent.map((c) => (
                    <li key={c.id} className="flex items-start gap-2 border-b border-line py-2.5">
                      <button
                        type="button"
                        onClick={() => cycleType(c)}
                        disabled={c.type === "pending" || c._optimistic}
                        className={`tap shrink-0 text-[11px] px-2 py-1 rounded border ${
                          c.type === "pending"
