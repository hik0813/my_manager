...
export default function TodayPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (i.ok) setHighlights(((await i.json()) as { highlights: FeedItem[] }).highlights);
      setError(null);
    } catch (e) {
  ...
  async function saveLog(withMood?: number) {
    if (!oneLiner.trim()) return;
    setSavingLog(true);
    await fetch("/api/logs", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ one_liner: oneLiner, mood: withMood ?? mood }),
    }).catch(() => undefined);
    setSavingLog(false);
    void load();
  }
  ...
  async function dismissAlerts(ids: string[]) {
    setData((d) => (d ? { ...d, alerts: d.alerts.filter((a) => !ids.includes(a.id)) } : d));
    ...
    <div>
      ...
      {unreadCritical.length > 0 && (
        <div className="mb-5 rounded-[10px] border border-danger/50 bg-danger/10 p-3">
          ...
          {/* 읽음 처리가 없으면 이 배너가 영원히 남는다 */}
          <button
            onClick={() => void dismissAlerts(unreadCritical.map((a) => a.id))}
            className="tap press mt-2 text-[12px] text-danger underline underline-offset-2"
          >
            확인함
          </button>
        </div>
      ...
      <Section title="오늘의 핵심" right={<span className="text-[11px] text-dim">점수순 상위 3</span>}>
        {highlights.length === 0 ? (
          <Empty>아직 올릴 만한 소식이 없다. 더보기 → 지금 수집을 누르면 바로 채워진다.</Empty>
        ) : (
          <ul>
            {highlights.map((h) => (
              <li key={h.id} className="border-b border-line">
                <button onClick={() => void openItem(h)} className="tap press text-left w-full flex gap-3 py-3">
                  <Thumb
                    src={h.image}
                    fallback={<span className="text-accent text-[15px] tabular-nums">{h.score}</span>}
                  />
                  <div className="min-w-0 flex-1">
                    <p className="text-[15px] leading-snug line-clamp-2">{h.title}</p>
                    {h.summary && <p className="text-[13px] text-muted mt-0.5 line-clamp-1">{h.summary}</p>}
                    <p className="text-[11px] text-dim mt-1 truncate">
                      {h.source_title} · {relKo(h.published_at ?? h.created_at)}
                    </p>
                  </div>
                </button>
              </li>
      ...
      {data.school && (
        <Section title="학교">
          ...
          ) : data.school.errors?.length ? (
            // NEIS가 왜 거절했는지 그대로 보여준다. 이걸 숨기면 원인을 찾을 방법이 없다.
            <div className="text-[13px]">
              <p className="text-danger">NEIS가 응답을 거절했다.</p>
              {data.school.errors.map((e, i) => (
                <p key={i} className="text-muted mt-0.5 break-all">
                  {e}
                </p>
              ))}
              <p className="text-dim mt-1">
                ERROR-290이면 인증키가 잘못된 것이다. open.neis.go.kr에서 받은 키를 NEIS_API_KEY에 넣고 재배포해라.
              </p>
            </div>
          ) : (
      ...
      {/* 하루 한 줄 — '오늘' 화면을 벗어나지 않고 입력한다.
          sticky로 띄웠더니 학교·할일 위에 겹쳐 그려져서 글자가 읽히지 않았다. 평범한 섹션으로 되돌린다. */}
      <section className="pt-2">
        ...
        {/* 기분은 히트맵 농도를 정하는데 입력할 자리가 없었다 — 저장하면 여기서 고른다 */}
        <div className="flex items-center gap-2 mt-1.5">
          {data.log && <span className="text-[11px] text-dim">오늘 기록됨</span>}
          <span className="text-[11px] text-dim">기분</span>
          {[1, 2, 3, 4, 5].map((m) => (
            <button
              key={m}
              aria-label={`기분 ${m}`}
              ...
              className="tap h-6 w-6 grid place-items-center"
            >
              <span
                className="h-2.5 w-2.5 rounded-full border"
                style={{
                  background: (data.log?.mood ?? mood) >= m ? "var(--color-accent)" : "transparent",
                  borderColor: (data.log?.mood ?? mood) >= m ? "var(--color-accent)" : "var(--color-line)",
                }}
              />
            </button>
          ))}
