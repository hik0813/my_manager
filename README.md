...
> **Hobby 플랜에서도 위 6개가 전부 돈다.** 크론 개수는 프로젝트당 100개까지 모든 플랜에서 허용되고, Hobby의 제약은 개수가 아니라 **크론 하나가 하루 한 번까지**라는 것이다. `0 * * * *` 같은 매시간 표현식은 배포 자체가 실패한다. 위 스케줄은 전부 하루 1회(주간 리포트는 주 1회)라 그대로 통과한다.
>
> 다만 Hobby는 **실행 시각 정밀도가 ±59분**이다. 06:00 수집은 06:00~06:59 사이, 07:00 브리핑은 07:00~07:59 사이 어딘가에 뜬다. 수집(21:00 UTC)이 브리핑(22:00 UTC)보다 항상 먼저 끝나도록 한 시간 간격을 띄워둔 이유다. 분 단위로 정확해야 하면 Pro가 필요하다. 급하면 더보기 화면의 "지금 수집" 버튼으로 언제든 수동 실행할 수 있다.
...
**홈화면 추가**: Safari에서 앱을 열고 공유 → 홈 화면에 추가. `display: standalone`이라 주소창 없이 전체화면으로 뜬다. 반드시 **Safari**로 해야 한다 — Chrome이나 카카오톡 인앱 브라우저에서는 홈 화면에 추가해도 그냥 북마크가 된다. 홈화면에 추가하기 전에 Safari에서 한 번 로그인해두면 쿠키가 넘어가서 앱 아이콘으로 열었을 때 바로 들어간다.

**알림**: iOS 웹 푸시는 홈화면 추가 상태에서만 되고 불안정해서 2차 채널로만 뒀다. 1차는 텔레그램이다. @BotFather로 봇을 만들어 토큰을 받고, 그 봇에게 아무 메시지나 한 번 보낸 뒤 `https://api.telegram.org/bot<토큰>/getUpdates` 를 열면 `chat.id`가 보인다. 그 둘을 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID`에 넣으면 끝이다.
